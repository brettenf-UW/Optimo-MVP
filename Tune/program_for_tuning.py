import pandas as pd
from pulp import LpMaximize, LpProblem, LpVariable, lpSum, LpStatus, PULP_CBC_CMD
import os
import json

class SchoolScheduler:
    def __init__(self, penalties):
        self.penalties = penalties
        # Load data
        self.load_data()
        # Initialize problem
        self.setup_problem()
        
    def load_data(self):
        """Load all input files"""
        input_dir = 'input'
        self.student_info = pd.read_csv(f'{input_dir}/Student_Info.csv')
        self.student_preference_info = pd.read_csv(f'{input_dir}/Student_Preference_Info.csv')
        self.teacher_info = pd.read_csv(f'{input_dir}/Teacher_Info.csv')
        self.teacher_unavailability = pd.read_csv(f'{input_dir}/Teacher_unavailability.csv')
        self.sections_info = pd.read_csv(f'{input_dir}/Sections_Information.csv')

    def setup_problem(self):
        """Initialize the LP problem with current penalties"""
        self.prob = LpProblem("School_Scheduling", LpMaximize)
        # Define periods
        self.periods = ['R1', 'R2', 'R3', 'R4', 'G1', 'G2', 'G3', 'G4']

        # Filter sections based on teacher availability and student requests
        self.valid_sections = []
        for section_id, teacher_id in zip(self.sections_info['Section ID'], self.sections_info['Teacher Assigned']):
            unavailable_periods = self.teacher_unavailability[self.teacher_unavailability['Teacher ID'] == teacher_id]['Unavailable Periods'].values
            if len(unavailable_periods) > 0 and pd.notna(unavailable_periods[0]):
                unavailable_periods = unavailable_periods[0].split(',')
            else:
                unavailable_periods = []
            for period in self.periods:
                if period not in unavailable_periods:
                    self.valid_sections.append((section_id, period))

        # Create a mapping from courses to their sections
        self.course_to_sections = {}
        for _, row in self.sections_info.iterrows():
            if row['Course ID'] not in self.course_to_sections:
                self.course_to_sections[row['Course ID']] = []
            self.course_to_sections[row['Course ID']].append(row['Section ID'])

        # Filter sections based on student requests
        self.requested_sections = set()
        for student_id in self.student_info['Student ID']:
            requested_courses = self.student_preference_info[
                self.student_preference_info['Student ID'] == student_id
            ]['Preferred Sections'].values[0].split(';')
            
            # For each requested course, add all its sections to the set
            for course_id in requested_courses:
                if (course_id in self.course_to_sections):
                    self.requested_sections.update(self.course_to_sections[course_id])

        # Create decision variables
        self.x = {}  # student-section assignments
        self.z = {}  # section-period assignments
        # New variable: student-section-period assignments
        self.y = {}

        for student_id in self.student_info['Student ID']:
            for section_id in self.requested_sections:
                self.x[(student_id, section_id)] = LpVariable(f"x_{student_id}_{section_id}", 0, 1, cat='Binary')
                for sec, period in self.valid_sections:
                    if sec == section_id:
                        self.y[(student_id, section_id, period)] = LpVariable(f"y_{student_id}_{section_id}_{period}", 0, 1, cat='Binary')

        for section_id, period in self.valid_sections:
            self.z[(section_id, period)] = LpVariable(f"z_{section_id}_{period}", 0, 1, cat='Binary')

        # Create diagnostic variables
        self.diagnostic_vars = {
            'missing_courses': {},      # Track students missing required courses
            'section_overload': {},     # Track overcrowded sections
            'period_conflicts': {},     # Track scheduling conflicts
            'sped_overload': {},       # Track SPED distribution issues
            'science_prep_violation': {},  # Track science prep period violations
            'medical_career_violation': {}, # Track Medical Career constraints
            'heroes_teach_violation': {},   # Track Heroes Teach constraints
            'sports_med_violation': {}      # Track Sports Med constraints
        }

        # Initialize base objective (maximizing assignments)
        self.objective = lpSum(self.x[(student_id, section_id)] 
                     for student_id in self.student_info['Student ID'] 
                     for section_id in self.requested_sections)

        # Add constraints with diagnostics

        # Add linking constraints between x and new y variables
        for student_id in self.student_info['Student ID']:
            for section_id in self.requested_sections:
                valid_periods = [period for sec, period in self.valid_sections if sec == section_id]
                self.prob += lpSum(self.y[(student_id, section_id, p)] for p in valid_periods) == self.x[(student_id, section_id)]

        # Ensure y is only active when the section is scheduled in that period
        for student_id in self.student_info['Student ID']:
            for section_id in self.requested_sections:
                valid_periods = [period for sec, period in self.valid_sections if sec == section_id]
                for p in valid_periods:
                    self.prob += self.y[(student_id, section_id, p)] <= self.z[(section_id, p)]

        # Replace the existing period constraint with a strict version
        for student_id in self.student_info['Student ID']:
            for period in self.periods:
                # Hard constraint: student can only be in one section per period
                self.prob += lpSum(
                    self.y[(student_id, section_id, period)]
                    for section_id in self.requested_sections
                    if (section_id, period) in self.valid_sections
                ) <= 1

        # 2. Each section must be scheduled in exactly one period
        for section_id in self.sections_info['Section ID']:
            self.prob += lpSum(self.z[(section_id, period)] 
                     for sec_id, period in self.valid_sections if sec_id == section_id) == 1

        # 3. Link student assignment to section scheduling
        for student_id in self.student_info['Student ID']:
            for section_id in self.requested_sections:
                self.prob += self.x[(student_id, section_id)] <= lpSum(self.z[(section_id, period)] 
                    for sec_id, period in self.valid_sections if sec_id == section_id)

        # 4. Section capacity constraints with diagnostics
        for section_id in self.sections_info['Section ID']:
            overload = LpVariable(f"section_overload_{section_id}", 0, None)
            self.diagnostic_vars['section_overload'][section_id] = overload
            
            capacity = self.sections_info[self.sections_info['Section ID'] == section_id]['# of Seats Available'].values[0]
            self.prob += lpSum(self.x[(student_id, section_id)] 
                     for student_id in self.student_info['Student ID'] 
                     if (student_id, section_id) in self.x) <= capacity + overload
            self.objective += -100 * overload

        # 5. One section per requested course constraint
        for student_id in self.student_info['Student ID']:
            requested_courses = self.student_preference_info[
                self.student_preference_info['Student ID'] == student_id
            ]['Preferred Sections'].values[0].split(';')
            
            for course_id in requested_courses:
                course_sections = self.course_to_sections.get(course_id, [])
                # Student can take at most one section of each course
                self.prob += lpSum(self.x[(student_id, section_id)] 
                         for section_id in course_sections
                         if (student_id, section_id) in self.x) <= 1

        # 6. Required courses constraint with diagnostics
        for student_id in self.student_info['Student ID']:
            requested_courses = self.student_preference_info[
                self.student_preference_info['Student ID'] == student_id
            ]['Preferred Sections'].values[0].split(';')
            
            for course_id in requested_courses:
                slack = LpVariable(f"missing_courses_{student_id}_{course_id}", 0, 1)
                self.diagnostic_vars['missing_courses'][(student_id, course_id)] = slack
                
                if course_id in self.course_to_sections:
                    course_sections = self.course_to_sections[course_id]
                    # Must get exactly one section of each course
                    self.prob += (lpSum(self.x[(student_id, section_id)] 
                            for section_id in course_sections 
                            if (student_id, section_id) in self.x) + slack == 1)
                    
                    self.objective += -1500 * slack  # High penalty for missing a required course

        # 7. Replace Medical Career constraints:
        medical_career_sections = self.sections_info[self.sections_info['Course ID'] == 'Medical Career']['Section ID']
        if not medical_career_sections.empty:
            r1_violation = LpVariable("medical_r1_violation", 0, None)
            g1_violation = LpVariable("medical_g1_violation", 0, None)
            self.diagnostic_vars['medical_career_violation'] = (r1_violation, g1_violation)
            self.prob += lpSum(self.z[(section_id, 'R1')] for section_id in medical_career_sections if (section_id, 'R1') in self.z) + r1_violation >= 1
            self.prob += lpSum(self.z[(section_id, 'G1')] for section_id in medical_career_sections if (section_id, 'G1') in self.z) + g1_violation >= 1
            self.objective += -800 * (r1_violation + g1_violation)

        # 8. Replace Heroes Teach constraints:
        heroes_teach_sections = self.sections_info[self.sections_info['Course ID'] == 'Heroes Teach']['Section ID']
        if not heroes_teach_sections.empty:
            r2_violation = LpVariable("heroes_r2_violation", 0, None)
            g2_violation = LpVariable("heroes_g2_violation", 0, None)
            self.diagnostic_vars['heroes_teach_violation'] = (r2_violation, g2_violation)
            self.prob += lpSum(self.z[(section_id, 'R2')] for section_id in heroes_teach_sections if (section_id, 'R2') in self.z) + r2_violation >= 1
            self.prob += lpSum(self.z[(section_id, 'G2')] for section_id in heroes_teach_sections if (section_id, 'G2') in self.z) + g2_violation >= 1
            self.objective += -800 * (r2_violation + g2_violation)

        # 9. Sports Med constraints with diagnostics
        sports_med_sections = self.sections_info[self.sections_info['Course ID'] == 'Sports Med']['Section ID']
        for period in self.periods:
            overlap = LpVariable(f"sports_med_overlap_{period}", 0, None)
            self.diagnostic_vars['sports_med_violation'][period] = overlap
            self.prob += lpSum(self.z[(section_id, period)] 
                     for section_id in sports_med_sections 
                     if (section_id, period) in self.valid_sections) <= 1 + overlap
            self.objective += -600 * overlap

        # 10. Science prep period constraints with diagnostics (modified for unique pairs)
        science_sections = self.sections_info[self.sections_info['Course ID'].str.contains('Science')]
        science_sections_ids = list(science_sections['Section ID'])
        for i in range(len(science_sections_ids)):
            for j in range(i+1, len(science_sections_ids)):
                section_id1 = science_sections_ids[i]
                section_id2 = science_sections_ids[j]
                course_id1 = self.sections_info[self.sections_info['Section ID'] == section_id1]['Course ID'].values[0]
                course_id2 = self.sections_info[self.sections_info['Section ID'] == section_id2]['Course ID'].values[0]
                if course_id1 != course_id2:
                    for period1, period2 in zip(self.periods[:-1], self.periods[1:]):
                        if period1 in ['R2', 'G2'] and period2 in ['R3', 'G3']:
                            continue
                        if (section_id1, period1) in self.z and (section_id2, period2) in self.z:
                            violation = LpVariable(f"science_prep_{section_id1}_{section_id2}_{period1}_{period2}", 0, None)
                            self.diagnostic_vars['science_prep_violation'][(section_id1, section_id2, period1, period2)] = violation
                            self.prob += self.z[(section_id1, period1)] + self.z[(section_id2, period2)] <= 1 + violation
                            self.objective += -400 * violation

        # Replace the existing teacher scheduling constraints with stricter ones
        for teacher in self.teacher_info['Teacher ID']:
            teacher_sections = list(self.sections_info[self.sections_info['Teacher Assigned'] == teacher]['Section ID'])
            for period in self.periods:
                # Force exactly 0 or 1 sections per teacher per period
                self.prob += lpSum(
                    self.z[(section_id, period)]
                    for section_id in teacher_sections
                    if (section_id, period) in self.z
                ) <= 1
                
                # Add a hard constraint that prevents multiple assignments
                for i in range(len(teacher_sections)):
                    for j in range(i + 1, len(teacher_sections)):
                        section1 = teacher_sections[i]
                        section2 = teacher_sections[j]
                        if (section1, period) in self.z and (section2, period) in self.z:
                            # Force that these two sections cannot be in the same period
                            self.prob += self.z[(section1, period)] + self.z[(section2, period)] <= 1

        # Identify SPED students
        sped_students = self.student_info[self.student_info['SPED'] == 1]['Student ID']

        # 11. SPED distribution constraints with diagnostics
        for section_id in self.sections_info['Section ID']:
            sped_overload = LpVariable(f"sped_overload_{section_id}", 0, None)
            self.diagnostic_vars['sped_overload'][section_id] = sped_overload
            self.prob += lpSum(self.x[(student_id, section_id)] 
                     for student_id in sped_students 
                     if (student_id, section_id) in self.x) <= 12 + sped_overload
            self.objective += -250 * sped_overload

        # Original balancing constraints using all student assignments:
        balance_weight = 50  # adjust this weight to tune balance encouragement

        for course_id, sections in self.course_to_sections.items():
            if len(sections) > 1:
                L_max = LpVariable(f"L_max_{course_id}", 0)
                L_min = LpVariable(f"L_min_{course_id}", 0)
                for sec in sections:
                    load_sec = lpSum(self.x[(student_id, sec)]
                                     for student_id in self.student_info['Student ID']
                                     if (student_id, sec) in self.x)
                    self.prob += L_max >= load_sec
                    self.prob += L_min <= load_sec
                self.objective += - balance_weight * (L_max - L_min)

        # Set the objective
        self.prob += self.objective

    def run(self):
        """Solve the problem and return conflicts"""
        solver = PULP_CBC_CMD(msg=False, timeLimit=30)
        self.prob.solve(solver)
        return self.analyze_conflicts()
        
    def analyze_conflicts(self):
        """Analyze solution for constraint violations"""
        conflicts = {
            'missing_courses': [],
            'teacher_conflicts': [],
            'special_course_issues': [],
            'overloaded_sections': []
        }
        # Check missing courses - now checking per course
        for (student_id, course_id), var in self.diagnostic_vars['missing_courses'].items():
            if var.varValue > 0:
                conflicts['missing_courses'].append({
                    'student': student_id,
                    'course': course_id,
                    'missing_count': 1
                })
    
        # Check section overloads
        for section_id, var in self.diagnostic_vars['section_overload'].items():
            if var.varValue > 0:
                course_id = self.sections_info[self.sections_info['Section ID'] == section_id]['Course ID'].values[0]
                conflicts['overloaded_sections'].append({
                    'section': section_id,
                    'course': course_id,
                    'overload': int(var.varValue)
                })
    
        # Check special course constraints
        # Medical Career
        for section_id, (r1_var, g1_var) in self.diagnostic_vars['medical_career_violation'].items():
            if r1_var.varValue > 0 or g1_var.varValue > 0:
                conflicts['special_course_issues'].append({
                    'type': 'Medical Career',
                    'section': section_id,
                    'issue': 'Not scheduled in required periods (R1/G1)'
                })
    
        # Heroes Teach
        for section_id, (r2_var, g2_var) in self.diagnostic_vars['heroes_teach_violation'].items():
            if r2_var.varValue > 0 or g2_var.varValue > 0:
                conflicts['special_course_issues'].append({
                    'type': 'Heroes Teach',
                    'section': section_id,
                    'issue': 'Not scheduled in required periods (R2/G2)'
                })
    
        # Sports Med
        for period, var in self.diagnostic_vars['sports_med_violation'].items():
            if var.varValue > 0:
                conflicts['special_course_issues'].append({
                    'type': 'Sports Med',
                    'period': period,
                    'issue': 'Sections overlap in same period'
                })
    
        # Teacher scheduling conflict analysis
        for teacher in self.teacher_info['Teacher ID']:
            teacher_sections = list(self.sections_info[self.sections_info['Teacher Assigned'] == teacher]['Section ID'])
            for period in self.periods:
                scheduled = sum(
                    self.z[(section_id, period)].varValue if self.z[(section_id, period)].varValue is not None else 0
                    for section_id in teacher_sections if (section_id, period) in self.z
                )
                if scheduled > 1:
                    conflicts['teacher_conflicts'].append({
                        'teacher': teacher,
                        'period': period,
                        'conflict_count': scheduled
                    })

        return conflicts
        
    def save_results(self, output_dir):
        """Save current solution files"""
        # Save schedule CSVs
        self.save_master_schedule(output_dir)
        self.save_student_assignments(output_dir)
        self.save_teacher_assignments(output_dir)

    def save_master_schedule(self, output_dir):
        """Save the master schedule to a CSV file"""
        section_scheduling = []
        for section_id, period in self.valid_sections:
            if self.z[(section_id, period)].varValue == 1:
                section_scheduling.append({'Section ID': section_id, 'Period': period})
        master_schedule_df = pd.DataFrame(section_scheduling)
        master_schedule_df.to_csv(os.path.join(output_dir, 'Master_Schedule.csv'), index=False)

    def save_student_assignments(self, output_dir):
        """Save student assignments to a CSV file"""
        student_assignments = []
        for student_id in self.student_info['Student ID']:
            for section_id in self.requested_sections:
                if (student_id, section_id) in self.x and self.x[(student_id, section_id)].varValue == 1:
                    student_assignments.append({'Student ID': student_id, 'Section ID': section_id})
        student_assignments_df = pd.DataFrame(student_assignments)
        student_assignments_df.to_csv(os.path.join(output_dir, 'Student_Assignments.csv'), index=False)

    def save_teacher_assignments(self, output_dir):
        """Save teacher assignments to a CSV file"""
        teacher_assignments = []
        for section_id, period in self.valid_sections:
            if self.z[(section_id, period)].varValue == 1:
                teacher_id = self.sections_info[self.sections_info['Section ID'] == section_id]['Teacher Assigned'].values[0]
                teacher_assignments.append({
                    'Teacher ID': teacher_id,
                    'Section ID': section_id,
                    'Period': period
                })
        teacher_assignments_df = pd.DataFrame(teacher_assignments)
        teacher_assignments_df.to_csv(os.path.join(output_dir, 'Teacher_Assignments.csv'), index=False)

# Example usage
penalties = {
    'missing_course': 1500,
    'section_overload': 100,
    'medical_career': 800,
    'heroes_teach': 800,
    'sports_med': 600,
    'science_prep': 400,
    'sped_overload': 250
}

scheduler = SchoolScheduler(penalties)
conflicts = scheduler.run()
scheduler.save_results('output')

def analyze_conflicts():
    """Analyze and report all constraint violations"""
    conflicts = scheduler.analyze_conflicts()
    
    print("\n=== Scheduling Conflict Analysis ===")
    print(f"\nSolver Status: {LpStatus[scheduler.prob.status]}")
    
    # Check missing courses - now checking per course
    missing_courses_found = False
    for conflict in conflicts['missing_courses']:
        missing_courses_found = True
    
    if missing_courses_found:
        print("\nStudents Missing Required Courses:")
        student_courses = {}
        for conflict in conflicts['missing_courses']:
            if conflict['student'] not in student_courses:
                student_courses[conflict['student']] = []
            student_courses[conflict['student']].append(conflict['course'])
            
        for student_id, missing_courses in student_courses.items():
            print(f"- Student {student_id} is missing {len(missing_courses)} courses:")
            for course in missing_courses:
                print(f"    - {course}")
    
    # Check section overloads
    overloads_found = False
    for conflict in conflicts['overloaded_sections']:
        overloads_found = True
    
    if overloads_found:
        print("\nOverloaded Sections:")
        for conflict in conflicts['overloaded_sections']:
            print(f"- Section {conflict['section']} ({conflict['course']}) is overloaded by {conflict['overload']} students")
    
    # Check special course constraints
    special_issues_found = False
    
    if conflicts['special_course_issues']:
        special_issues_found = True
    
    if special_issues_found:
        print("\nSpecial Course Scheduling Issues:")
        for conflict in conflicts['special_course_issues']:
            print(f"- {conflict['type']}: {conflict['issue']}")
    
    # Generate recommendations
    print("\n=== Recommendations ===")
    
    if conflicts['missing_courses']:
        print("\nTo resolve missing courses:")
        course_counts = Counter([
            conflict['course']
            for conflict in conflicts['missing_courses']
        ])
        for course, count in course_counts.most_common():
            print(f"- Add capacity for {count} more students in {course}")
    
    if conflicts['overloaded_sections']:
        print("\nTo resolve section overloads:")
        for conflict in conflicts['overloaded_sections']:
            print(f"- Increase capacity of {conflict['course']} section {conflict['section']} by {conflict['overload']} seats")
            print(f"  OR add new section to accommodate {conflict['overload']} students")
    
    if conflicts['special_course_issues']:
        print("\nTo resolve special course issues:")
        for conflict in conflicts['special_course_issues']:
            if conflict['type'] == 'Medical Career':
                print(f"- Ensure Medical Career section {conflict['section']} is scheduled in both R1 and G1")
            elif conflict['type'] == 'Heroes Teach':
                print(f"- Ensure Heroes Teach section {conflict['section']} is scheduled in both R2 and G2")
            elif conflict['type'] == 'Sports Med':
                print(f"- Resolve Sports Med sections overlap in period {conflict['period']}")

    # Teacher scheduling conflict analysis
    teacher_conflicts = conflicts['teacher_conflicts']
    
    if teacher_conflicts:
        print("\nTeacher Scheduling Conflicts:")
        for conflict in teacher_conflicts:
            print(f"- Teacher {conflict['teacher']} has {conflict['conflict_count']} sections scheduled in period {conflict['period']}")

    return conflicts

# Output results and create CSV files
# Create output directory
output_dir = 'output'
os.makedirs(output_dir, exist_ok=True)

# Output section scheduling
print("\nSection Scheduling:")
section_scheduling = []
for section_id, period in scheduler.valid_sections:
    if scheduler.z[(section_id, period)].varValue == 1:
        print(f"Section {section_id} is scheduled in period {period}")
        section_scheduling.append({'Section ID': section_id, 'Period': period})

# Output the master schedule
master_schedule_df = pd.DataFrame(section_scheduling)
master_schedule_df.to_csv(os.path.join(output_dir, 'Master_Schedule.csv'), index=False)
print("\nMaster Schedule:")
print(master_schedule_df)

# Output student assignments
print("\nStudent Assignments:")
student_assignments = []
for student_id in scheduler.student_info['Student ID']:
    assigned_sections = []
    for section_id in scheduler.requested_sections:
        if (student_id, section_id) in scheduler.x and scheduler.x[(student_id, section_id)].varValue == 1:
            assigned_sections.append(section_id)
            student_assignments.append({'Student ID': student_id, 'Section ID': section_id})
    if assigned_sections:
        print(f"Student {student_id} is assigned to sections: {assigned_sections}")
    else:
        print(f"Student {student_id} is not assigned to any section")

# Create student assignments CSV
student_assignments_df = pd.DataFrame(student_assignments)
student_assignments_df.to_csv(os.path.join(output_dir, 'Student_Assignments.csv'), index=False)

# Output teacher assignments
teacher_assignments = []
for section_id, period in scheduler.valid_sections:
    if scheduler.z[(section_id, period)].varValue == 1:
        teacher_id = scheduler.sections_info[scheduler.sections_info['Section ID'] == section_id]['Teacher Assigned'].values[0]
        teacher_assignments.append({
            'Teacher ID': teacher_id,
            'Section ID': section_id,
            'Period': period
        })

# Create teacher assignments CSV
teacher_assignments_df = pd.DataFrame(teacher_assignments)
teacher_assignments_df.to_csv(os.path.join(output_dir, 'Teacher_Assignments.csv'), index=False)

# Output unmet requests
students_unmet_requests = []
for student_id in scheduler.student_info['Student ID']:
    requested_courses = scheduler.student_preference_info[
        scheduler.student_preference_info['Student ID'] == student_id
    ]['Preferred Sections'].values[0].split(';')
    
    assigned_courses = set()
    for section_id in scheduler.requested_sections:
        if (student_id, section_id) in scheduler.x and scheduler.x[(student_id, section_id)].varValue == 1:
            course_id = scheduler.sections_info[
                scheduler.sections_info['Section ID'] == section_id
            ]['Course ID'].values[0]
            assigned_courses.add(course_id)
    
    unmet_courses = set(requested_courses) - assigned_courses
    if unmet_courses:
        students_unmet_requests.append({
            'Student ID': student_id,
            'Unmet Requests': ','.join(unmet_courses)
        })

# Create unmet requests CSV
students_unmet_requests_df = pd.DataFrame(students_unmet_requests)
students_unmet_requests_df.to_csv(os.path.join(output_dir, 'Students_Unmet_Requests.csv'), index=False)
print("\nStudents Without All Requested Courses:")
print(students_unmet_requests_df)

# Run the conflict analysis
analyze_conflicts()